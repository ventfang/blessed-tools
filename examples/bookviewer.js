...
var table = grid.set(0,0,3,5,contrib.table,
   ...
   , columnSpacing: 5
   , columnWidth: [12, 12, 12, 12, 15, 12, 12, 20, 12]})
...
function genBookView(book, pair) {
    ...
    var asks = Object.values(depth['asks']).sort((b, a) => { return Number.parseFloat(a[1]) - Number.parseFloat(b[1]) }).slice(0,20);
    var bids = Object.values(depth['bids']).sort((b, a) => { return Number.parseFloat(a[1]) - Number.parseFloat(b[1]) }).slice(0,20);
    if (asks.length < 20) {
    ...
    asks.map((d) => {
        ...
        rows.push([d[0].padStart(d[0].length-colors.stripColors(d[0]).length+5), fmtItem(ticks,decimal1,colors.stripColors(d[0])=='0'?0:-1), fmtItem(d[2],decimal2,0)]);
